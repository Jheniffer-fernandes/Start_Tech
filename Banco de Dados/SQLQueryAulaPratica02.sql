...
/* Só clientes do sexo masculino*/

SELECT * FROM clientes WHERE sexo = 'm';

/* Só clientes do sexo feminino acima de 30 anos que seja ordenado da mais velha ao mais nova*/
...
SELECT COUNT(*) AS contagem_fuincionario FROM funcionarios WHERE sexo = 'm';

/* Ordenar os funcionários por ordem de nascimento */

SELECT * FROM funcionarios ORDER BY data_nascimento ASC;
...
/*Qual sexo ganha mais em média e quantas pessoas tem por sexo? */

SELECT sexo, AVG(salario) AS media, COUNT(id) AS funcionarios FROM funcionarios GROUP BY sexo;